...
# @param burnin The number of burnin (warmup) iterations. Should only be
#   specified if the burnin samples are included in \code{X}.
...
#   dimension. See Examples in \code{\link[shinyStan]{as.shinystan}}.
# @param model_code A character string with the code you used to run your model.
#   This can also be added to your \code{shinystan} object later using the 
#   \code{\link[shinyStan]{include_model_code}} function. See
#   \code{\link[shinyStan]{include_model_code}} for additional formatting
#   instructions. After launching the app \code{model_code} will be viewable in
#   the \strong{Model Code} tab.
#   
# @return An object of class \code{shinystan} that can be used with 
#   \code{\link[shinystan]{launch_shinystan}}.
# 

mcmc2shinystan <- function(X, model_name = "unnamed model", burnin = 0, 
                           param_dims = list(), model_code) {
  coda_check()
  ...
  samps_array <- array(NA, 
                       dim = c(coda::niter(X), coda::nvar(X), coda::nchain(X)),
                       dimnames = list(iter = time(X), var = coda::varnames(X), 
