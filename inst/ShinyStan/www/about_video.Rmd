...
check_requirements <- function() {
  x <- list()
  x$image_magick <- ifelse(system("ffmpeg --help")==0," present"," not present")
  x$gg_animate <- ifelse(require(gganimate)," present"," not present")
  return(x)
}
checks <- check_requirements()
```

Your system shows that the ffmpeg library is `r checks$image_magick` and the gganimate package is `r checks$gg_animate`. If either of these are listed as "not present", please read the installation instructions below.
...
The videos are produced in the .WEBM format. This format may not display in Safari or Internet Explorer without additional plugins; however, it will play correctly on Chrome and Firefox. The movie can also be uploaded directly to Youtube to share and/or watch. To do so, click the download Movie button at the bottom of the screen to download the movie to your computer.
