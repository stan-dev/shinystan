...
function(input, output, session) {
  ...
  observeEvent(input$save_and_close_button, stopApp(object))
  ...
  observe({
    ...
    lapply(seq_along(options_inputs), function(j) {
      shinyjs::onclick(
        paste0(options_inputs[j], "_options_show"),
        shinyjs::toggle(
          id = paste0(options_inputs[j], "_options"),
          anim = TRUE,
          animType = "slide",
          time = 0.4
        )
  ...
  observe({
    ...
    local({
      if (diag_nav != 'By model parameter')
        lapply(diagnostic_inputs, function(x)
  ...
  observeEvent(input$open_quick_rhat, shinyjs::info(includeText("text/quick_rhat.txt")))
  observeEvent(input$open_quick_neff, shinyjs::info(includeText("text/quick_neff.txt")))
  observeEvent(input$open_quick_mcse, shinyjs::info(includeText("text/quick_mcse.txt")))
