    ...
    fluidRow(
      ...
      wellPanel(id = "selectVariableTab",
                fluidRow(h4("Please note that you need to visit the tabs, and, if applicable, generate the plots before you can download the report."),
                         br()),
                fluidRow(
                  if(shinystan:::.sso_env$.SHINYSTAN_OBJECT@misc$stan_method == "sampling" & shinystan:::.sso_env$.SHINYSTAN_OBJECT@misc$stan_algorithm == "NUTS") h4("NUTS/HMC Diagnostics Options"),
                  if(shinystan:::.sso_env$.SHINYSTAN_OBJECT@misc$stan_method == "sampling" & shinystan:::.sso_env$.SHINYSTAN_OBJECT@misc$stan_algorithm == "NUTS") column(width = 4, 
                  ...
                  if(shinystan:::.sso_env$.SHINYSTAN_OBJECT@misc$stan_method == "sampling" & shinystan:::.sso_env$.SHINYSTAN_OBJECT@misc$stan_algorithm == "NUTS") column(width = 4, 
                         ...
                         radioButtons(ns("treedepth"), "Include Treedepth Plot",
                                      choices = c(TRUE, FALSE), selected = FALSE))),
                fluidRow(
                if(shinystan:::.sso_env$.SHINYSTAN_OBJECT@misc$stan_method == "sampling") h4("MCMC Diagnostics Options"),
                if(shinystan:::.sso_env$.SHINYSTAN_OBJECT@misc$stan_method == "sampling") column(width = 4, 
