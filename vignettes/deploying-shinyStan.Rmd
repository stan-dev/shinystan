...
```r
deploy_shinystan(sso, appName = "MyModel", account = "username")
```
...
```r
deploy_shinystan(sso, appName = "MyModel")
```


**Using the `Dir` argument**

When you deploy a shinyStan app a folder containing the contents of the app required for deployment will be created on your system. The name of the folder created will always be the name you use in the `appName` argument. By default the folder will created in the working directory, but you can specify a different location using the `Dir` argument:

```r
deploy_shinystan(sso, appName = "MyModel", Dir = "/Users/username/Desktop")
```

This will create a folder called `MyModel` on the Desktop. After the app is deployed and viewable online you can keep or delete the `MyModel` folder without affecting the deployed app.  
...
```r
deploy_shinystan(sso, appName = "MyModel", ppcheck_data = y)
```
...
```r
deploy_shinystan(sso, appName = "MyModel", ppcheck_data = y, ppcheck_yrep = "yRep")
```
