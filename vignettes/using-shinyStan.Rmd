...
by using the `include_model_code` function. For example, you may have used Bugs 
...
`code` argument to the `include_model_code` function 
...
sso <- include_model_code(sso, code = my_code)
```
...
`shinystan` object you can use the `rename_model` function like this:

```r
sso <- rename_model(sso, "new_model_name")
```
