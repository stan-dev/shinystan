...
library(shinyStan)
library(rstan)
my_stanfit <- stan(file = "my_stan_code.stan", 
                   data = my_stan_data)
```

Using **shinyStan** with `my_stanfit` is as easy as
```{r, rstan_shinystan, eval=FALSE}
...
To convert a 3-D array to a `shinystan` object make sure that the 3 dimensions of the array correspond to the *number of iterations* **x** *number of chains*  **x** *number of parameters*. You can then `as.shinystan` exactly how it's used in the examples for `mcmc.list` objects above (but you don't need to convert your array to an `mcmc.list`).
